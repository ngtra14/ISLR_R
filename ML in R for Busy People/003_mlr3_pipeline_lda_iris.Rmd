...
learner_1$train(task, row_ids = train_index)
```
...
prediction = learner_1$predict(task, row_ids = train_index )
print(prediction)
```
```{r}
# predict using "new" observations from an external data.frame
prediction = learner_1$predict_newdata(newdata = iris[test_index, ])
print(prediction)
```
## Evaluation
