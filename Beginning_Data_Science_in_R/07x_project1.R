...
# write.csv(wines, "wines.csv")
## ----wine-qualities,                "Distribution of wine qualities."----
ggplot(wines) + 
...
## ------------------------------------------------------------------------
wines$type <- as.factor(wines$type) # fix the problem by converting type
random_wines <- wines %>% 
    partition(4, c(training = 0.5, test = 0.5))
random_wines %>% prediction_accuracy_wines # somehow, not working this line

## ---- echo=FALSE, warning=FALSE------------------------------------------
suppressPackageStartupMessages(library(party, quietly = TRUE))

## ---- message=FALSE,"Decision tree for determining the type of wine."----
...
test_and_training %>% prediction_accuracy_wines(ctree)
