...
## ----dist-vs-speed,"Plot of breaking distance versus speed for cars."----
...
## ----lin-regr "Prediction lines for different choices of parameters."----
...
## ----lin-regress  "Error values for different choices of parameters."----
...
## ----lin-regress-best-    "Best regression line going through (0,0)."----
cars %>% ggplot(aes(x = speed, y = dist)) +
  geom_point() +
...
## ----breast_cancer   "Logistic regression fit to breast cancer data."----
BreastCancer %>% 
...
grouped_cars[[1]] # need a random seed ?
...
estimates # equivalent to 'for loops'
...
predicted_class <- predict(model, BreastCancer) %>% 
  as.data.frame %>%
  mutate(benign = ifelse(benign > 0.5, "benign", "malignant")) # add mutate

table(BreastCancer$Class, predicted_class$benign) # $benign added
