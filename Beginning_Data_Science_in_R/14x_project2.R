...
## ----"Weight vectors sampled from the prior distribution.",   ----
...
## ----"Randomly sampled $(x,y)$ values.  --------------------------
## ----- The true linear model is shown in red."----
w0 <- 0.3 ; w1 <- 1.1 ; beta <- 1.3
x <- rnorm(50)
y <- rnorm(50, w1 * x + w0, 1/beta)

## ----  "Randomly sampled $(x,y)$ values.", echo=FALSE-------------
plot(x, y, pch=20)
...
## ---- The true value is shown as a red dot." ----
...
## ----True linear model in red and predicted values in blue."----
...
dev.off()
## ---- "Prediction with 95% support interval." --------
...
## ----"Prediction with 95% support interval, wider range." ----
predict_new_quantile <- function(x, q, posterior, beta) {
  predict_dist <- predict_new_dist(x, posterior, beta)
  qnorm(q, mean = predict_dist$mean, sd = sqrt(predict_dist$var))
}
